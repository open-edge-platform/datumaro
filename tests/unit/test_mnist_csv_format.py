...
class MnistCsvFormatTest(TestCase):
    ...
    def test_can_save_and_load_with_different_image_size(self):
        source_dataset = Dataset.from_iterable(
            [
                DatasetItem(id=0, media=Image.from_numpy(data=np.ones((10, 8))), annotations=[Label(0)]),
                DatasetItem(id=1, media=Image.from_numpy(data=np.ones((4, 3))), annotations=[Label(1)]),
            ],
            categories={
                AnnotationType.label: LabelCategories.from_iterable(str(label) for label in range(10)),
            },
    ...
    def test_can_save_and_load_with_meta_file(self):
        ...
        with TestDir() as test_dir:
            MnistCsvExporter.convert(source_dataset, test_dir, save_media=True, save_dataset_meta=True)
            parsed_dataset = Dataset.import_from(test_dir, "mnist_csv")
