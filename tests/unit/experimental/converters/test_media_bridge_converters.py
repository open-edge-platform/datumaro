...
class ImageInfoToMediaInfoConverterTest:
    ...
    def test_convert_multiple_rows(self):
        ...
        assert result["media_info"][0]["width"] == 640
        assert result["media_info"][0]["height"] == 480
        assert result["media_info"][1]["width"] == 1920
        assert result["media_info"][1]["height"] == 1080
...
class MediaInfoToImageInfoConverterTest:
    ...
    def test_convert_multiple_rows(self):
        """Test with multiple rows of mixed image and video frame data."""
        conv = MediaInfoToImageInfoConverter()
        ...
        assert result["image_info"][0]["width"] == 640
        assert result["image_info"][0]["height"] == 480
        assert result["image_info"][1]["width"] == 1920
        assert result["image_info"][1]["height"] == 1080


class ConverterRegistrationTest:
    """Tests that verify the new converters are properly registered."""

    def test_all_new_converters_registered(self):
        """Verify all new converters appear in the registry."""
        from datumaro.experimental.converters.registry import ConverterRegistry

        registered = {cls.__name__ for cls in ConverterRegistry.list_converters()}

        assert "ImagePathToImageInfoConverter" in registered
        assert "ImagePathToImageCallableConverter" in registered
        assert "VideoFrameCallableToImageCallableConverter" in registered
        assert "ImagePathToMediaPathConverter" in registered
        assert "ImageInfoToMediaInfoConverter" in registered
        assert "MediaPathToImagePathConverter" in registered
        assert "MediaInfoToImageInfoConverter" in registered

    def test_image_path_to_info_not_lazy(self):
        """ImagePathToImageInfoConverter should NOT be lazy."""
        assert ImagePathToImageInfoConverter.lazy is False

    def test_image_path_to_callable_not_lazy(self):
        """ImagePathToImageCallableConverter should NOT be lazy."""
        assert ImagePathToImageCallableConverter.lazy is False

    def test_video_frame_callable_converter_not_lazy(self):
        """VideoFrameCallableToImageCallableConverter should NOT be lazy."""
        assert VideoFrameCallableToImageCallableConverter.lazy is False

    def test_image_path_to_media_path_not_lazy(self):
        """ImagePathToMediaPathConverter should NOT be lazy."""
        assert ImagePathToMediaPathConverter.lazy is False

    def test_image_info_to_media_info_not_lazy(self):
        """ImageInfoToMediaInfoConverter should NOT be lazy."""
        assert ImageInfoToMediaInfoConverter.lazy is False

    def test_media_path_to_image_path_not_lazy(self):
        """MediaPathToImagePathConverter should NOT be lazy."""
        assert MediaPathToImagePathConverter.lazy is False

    def test_media_info_to_image_info_not_lazy(self):
        """MediaInfoToImageInfoConverter should NOT be lazy."""
        assert MediaInfoToImageInfoConverter.lazy is False
