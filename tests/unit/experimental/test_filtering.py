...
def test_empty_bbox_filter(empty_annotations_df):
    ...
    schema = Schema(
        {
            "bboxes": AttributeInfo(type=np.ndarray, annotation=bbox_field(dtype=pl.Float64)),
        }
    )
...
def test_empty_polygon_filter(empty_annotations_df):
    ...
    schema = Schema(
        {
            "polygons": AttributeInfo(type=np.ndarray, annotation=polygon_field(dtype=pl.Float64)),
        }
    )
