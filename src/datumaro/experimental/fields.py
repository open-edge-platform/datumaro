...
class NumericField(Field):
    """
    Represents a neutral numeric value (not specifically a score).

    Use this for generic per-sample or per-instance numeric attributes such as
    area, depth, distance, etc. Supports scalar values or lists of values when
    ``is_list=True``.
    """
...
class BoolField(Field):
    ...
    Useful for flags such as ``iscrowd``. Stored as Polars Boolean type and
...
class ImageIdField(Field):
    ...
    semantic: Semantic
    dtype: PolarsDataType = field(default_factory=pl.Int32)
...
def image_id_field(dtype: PolarsDataType = pl.Int32(), semantic: Semantic = Semantic.Default) -> Any:
    """
